        ...
        <el-col :span="6">
          <el-form-item label="接收仓类型：" prop="receiveWarehouseType">
            <el-input
              ...
              disabled
              :value="form.receiveWarehouseTypeName"
            />
            <!-- <BjSelectLov
    ...
    <el-row type="flex">
      ...
      <el-col :span="11">
        <p class="title">
          ...
          <el-button type="primary" @click="onSet">仓库结构比例设置</el-button>
        </p>
        <el-table
          ref="tableReceive"
        ...
        >
          ...
          <!-- <el-table-column prop="qty" label="仓位数量">
            <template slot-scope="props">
              <el-input :maxlength="MAX_LENGTH.DEFAULT"    v-model="props.row.qty"  />
            </template>
          </el-table-column> -->
        </el-table>
      </el-col>
...
export default {
  name: 'BusinessStockDistributionRatioAddSku',
  components: {},
  mixins: [viewMiXin],
  data() {
    return {
  ...
  methods: {
    // 跳转到仓库结构设置
    onSet() {
      stockApi.getWarehouseStructureId().then(res => {
        if (res.id || res._token || res.content || Array.isArray(res) || res.failed === false) {
          this.$toPage('SettingStockRatioConfig', { structureId: res.id });
        }
      });
    },
    ...
    getGoodsReceive(API_DATA) {
      stockApi.getDistributeSpacesGoods(API_DATA).then(res => {
        ...
        if (res.number === 0) {
          this.tableDataReceive = res.content || [];
        } else {
          this.tableDataReceive = this.tableDataReceive.concat(res.content || []);
        }
      });
    },
    // 接收仓的仓库列表
