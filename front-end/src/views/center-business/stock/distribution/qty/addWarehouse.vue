<template>
  <div class="app-container">
      ...
      <el-row :gutter="10">
        <el-col :span="6">
          <el-form-item label="供货仓类型：" prop="supportWarehouseType">
            <el-input :maxlength="MAX_LENGTH.DEFAULT" disabled value="实仓" />
          </el-form-item>
        </el-col>
        <el-col :span="6">
    ...
    <BjDivider
      >接收仓库
      <template slot="button">
        <el-button type="primary" @click="dialogListWarehouseShow('qty-virtual')"
          >添加数量虚仓</el-button
        >
        <el-button type="primary" @click="dialogListWarehouseShow('interline-virtual')"
          >添加中转虚仓</el-button
        >
      </template>
    </BjDivider>
    ...
    >
      ...
      <el-table-column type="index" width="55" align="center" label="序号" />
      <el-table-column prop="receiveTypeCode" label="仓库类型">
        <template slot-scope="scope">
          {{ $store.state.values.STOCK_WAREHOUSE_TYPE.bjGet(scope.row.receiveTypeCode) }}
        </template>
      </el-table-column>
      <el-table-column prop="receiveCode" label="仓库编码" />
      <el-table-column prop="receiveName" label="仓库名称" />

      <el-table-column prop="distributionQty" label="仓位数量">
        <template slot-scope="props">
          <el-input v-model="props.row.distributionQty" :maxlength="MAX_LENGTH.DEFAULT" />
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="100" align="center">
        <template slot-scope="scope">
...
export default {
  name: 'BusinessStockDistributionQtyAddWarehouse',
  components: {},
  mixins: [viewMiXin],
  data() {
    return {
      ...
      dialogListWarehouseConfig: {
        ...
        search: [
          {
            placeholder: '请输入接收仓编码',
            key: 'receiveWarehouseCode',
          },
          {
            placeholder: '请输入接收仓名称',
  ...
  methods: {
    // 显示 选择接收仓
    dialogListWarehouseShow(receiveTypeCode) {
      this.$refs['form'].validate(valid => {
        if (!valid) return this.$scrollToFormError();
        this.dialogListWarehouseConfig.defaultQuery.receiveTypeCode = receiveTypeCode;
        this.dialogListWarehouseConfig.defaultQuery.supportWarehouseId =
          this.form.supportWarehouseId;
        this.$refs.dialogListWarehouse.show();
      });
    },
    // 显示 选择商品
    dialogListGoodsShow() {
    ...
    onConfirmDialogListGoods(data) {
      ...
      this.form.skuCode = this.tableDataSku[0].skuCode;
      this.loadReceive(1);
    ...
    onConfirmDialogList(data) {
      data.forEach(row => {
        if (!this.tableDataWarehouse.find(e => e.receiveCode === row.code)) {
          this.tableDataWarehouse.push({
            id: null,
            receiveCode: row.code,
            receiveName: row.name,
            receiveTypeCode: row.typeCode,
            receiveWarehouseId: row.id,
            distributionQty: null,
            // distributionQty: 0
            // distributionType: 1
            // id: "=SZEBKlJ6G_w5qe5COTxjgbZ1ZZo3VBON2vxM_0Qh-Mo=="
            // receiveCode: "CO00000265"
            // receiveName: "数量虚仓2"
            // receiveTypeCode: "qty-virtual"
            // receiveTypeMeaning: null
            // receiveWarehouseId: "=F58CGTkdxPaEd9pHMxZWDcCthUJ2yXh72WthwRonm3U=="
            // structureDistributionQty: null
          });
        }
      });
    },
    ...
    getWarehousesReceive(API_DATA) {
      stockApi.getDistributeSpacesWarehouse(API_DATA).then(res => {
        ...
        const DATA = (res.content || []).map(e => {
          return {
            ...e,
            distributionQty: this.$infinityNumber(e.distributionQty),
          };
        });
        if (res.number === 0) {
          this.tableDataWarehouse = [].concat(DATA);
        } else {
          this.tableDataWarehouse = this.tableDataWarehouse.concat(DATA);
        }
      });
    },
    handleConfirm() {
      ...
      const API_DATA = {
        freightSpaceList: this.tableDataWarehouse.map(e => {
          return {
            qty: this.$infinityNumber(e.distributionQty),
            skuCode: e.skuCode,
            receiveWarehouseId: e.receiveWarehouseId,
          };
        }),
        skuId: this.tableDataSku[0].skuId,
        skuCode: this.tableDataSku[0].skuCode,
        supportWarehouseId: this.form.supportWarehouseId,
      };
    ...
    addApi(API_DATA) {
      stockApi.addBatchDistributeSpacesQty(API_DATA).then(res => {
