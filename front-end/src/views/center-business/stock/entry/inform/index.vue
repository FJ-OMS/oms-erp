        ...
        <el-col :span="colSpan">
          <el-form-item label="单号" prop="queryCode">
            <BjInputCode
              ...
              placeholder="入库通知单号/三方订单号/内部单号/前置单号"
...
export default {
  ...
  data() {
    return {
      ...
      queryApi: stockApi.getEntryInform,
      // 查询参数
      queryParams: {
        queryCode: null,
      ...
      labelsVersion: 11,
      labelList: [
        { label: '入库通知单号', prop: 'code', isCheck: true, disabled: false, width: '160' },
        {
          label: '三方订单号',
        ...
        { label: '需入库数量', prop: 'qty', isCheck: true, disabled: false, width: '100' },
        {
          label: '入库逻辑仓编码',
  ...
  methods: {
    ...
    onView(row, type) {
      ...
      if (type === 'platformOrderCode') {
        this.$toBusinessOrderThirdView(row[type]);
      }
      /** 内部单详情 */
      if (type === 'orderCode') {
        this.$toBusinessOrderInternalView(row[type]);
      }
