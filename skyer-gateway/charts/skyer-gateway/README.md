...
```bash
$ helm install c7n/skyer-gateway --name skyer-gateway
```
...
`image.repository` | 镜像库地址 | `registry.skyer.com.cn/skyer/skyer-gateway`
...
`service.name` | service名称 | `skyer-gateway`
...
```bash
$ helm install c7n/skyer-gateway \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=skyer-gateway  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
    --name skyer-gateway
```

## 验证部署
```bash
curl -s $(kubectl get po -n c7n-system -l skyer.io/release=skyer-gateway -o jsonpath="{.items[0].status.podIP}"):8081/actuator/health | jq -r .status
```
出现以下类似信息即为成功部署
