  ...
  <div class="app-container">
    ...
    >
      ...
      <template v-slot:tableOperation>
        <el-col>
          <el-button
            v-permission:BusinessAfterThirdReturnAdd
            type="primary"
            class="main-btn"
            @click="command('新建退货退款单')"
            >新建退货退款单</el-button
          >
          ...
          >
            <template slot-scope="scope">
              ...
              <template v-else-if="col.prop === 'outRefundNo'">
                <span
                  class="link"
                  @click="$toBusinessAfterThirdReturnView(scope.row.outRefundNo)"
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        storeIdList: [],
        startTime: null, // 三方退货/退货时间 ,
        endTime: null, // 三方退货/退货时间 ,
        orderNo: null, // 第三方/内部订单号 ,
        channelTypeCode: null,
        buyerNick: null, // 买家昵称
        exceptionStatus: null, // 转单状态 0- 正常 1-异常 tab页切换 ,
        type: AFTER_ORDER_TYPES.REFUND_GOODS, // 类型 1-仅退货单 2- 退货退货
      },
      labelsVersion: 11,
      labelList: [
        {
        ...
        { label: '退货原因', prop: 'reason', isCheck: true, disabled: false },
        { label: '退货备注', prop: 'remarks', isCheck: true, disabled: false },
  ...
  methods: {
    ...
    command(action, row) {
      switch (action) {
        case '新建退货退款单':
          this.$router.push({ name: 'BusinessAfterThirdReturnAdd' });
          break;
        case '导入':
          break;
        case '导出':
